...
def cell_assembly_detection(data, maxlag, reference_lag=2, alpha=0.05,
    ...
    # TODO: to change to get assemblies bigger
    ...
    while w1 < n_as:
        # print(f"w1 < n_as: {w1} < {n_as}")
        # print(f"current_size_agglomeration {current_size_agglomeration}")
        ...
        # print(f"assembly[w1]['neurons'] {assembly[w1]['neurons']}, w2_to_test {w2_to_test}")
        if w2_to_test:
            ...
            for ww2 in range(len(w2_to_test)):
                ...
                # if current_size_agglomeration == 3:
                #     print(f"{len(assem_tp['neurons'])} assem_tp['neurons'] {assem_tp['neurons']}")
                #     print(f"alpha {alpha}")
                #     print(f"assem_tp['pvalue'][-1] {assem_tp['pvalue'][-1]}")
                #     print(f"min_occ assem_tp['signature'][-1][1] {assem_tp['signature'][-1][1]}")
                #     print(f"max_spikes = {max_spikes} assem_tp['signature'][-1][0] {assem_tp['signature'][-1][0]}")
                if assem_tp['pvalue'][-1] < alpha and \
                        assem_tp['signature'][-1][1] > min_occ and \
                        assem_tp['signature'][-1][0] <= max_spikes:
                    # the assembly is saved in the output list of
                    # assemblies
                    # print(f"Passed: {len(assem_tp['neurons'])} assem_tp['neurons'] {assem_tp['neurons']}")
                    assembly.append(assem_tp)
                    assembly_flag = 1
